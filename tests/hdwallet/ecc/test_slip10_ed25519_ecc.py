...
def test_slip10_ed25519_ecc():

    assert SLIP10Ed25519ECC.NAME == _["SLIP10-Ed25519"]["name"]
...
def test_slip10_ed25519_ecc_point():

    assert SLIP10Ed25519Point.name() == _["SLIP10-Ed25519"]["name"]
    for public_key_type in ["uncompressed", "compressed"]:
        # Test from bytes
        point = SLIP10Ed25519Point.from_bytes(
            get_bytes(_["SLIP10-Ed25519"][public_key_type]["point"]["encode"])
        )
        assert isinstance(point, IPoint)
        assert isinstance(point, SLIP10Ed25519Point)
        assert isinstance(point.underlying_object(), bytes)
        assert point.x() == _["SLIP10-Ed25519"][public_key_type]["point"]["x"]
        assert point.y() == _["SLIP10-Ed25519"][public_key_type]["point"]["y"]
        assert point.raw_encoded() == get_bytes(_["SLIP10-Ed25519"][public_key_type]["point"]["encode"])
        assert point.raw() == point.raw_decoded() == get_bytes(_["SLIP10-Ed25519"][public_key_type]["point"]["decode"])
        # Test from coordinate
        point = SLIP10Ed25519Point.from_coordinates(
            x=_["SLIP10-Ed25519"][public_key_type]["point"]["x"],
            y=_["SLIP10-Ed25519"][public_key_type]["point"]["y"]
        )
        assert isinstance(point, IPoint)
        assert isinstance(point, SLIP10Ed25519Point)
        assert isinstance(point.underlying_object(), bytes)
...
def test_slip10_ed25519_ecc_public_key():

    assert SLIP10Ed25519PublicKey.name() == _["SLIP10-Ed25519"]["name"]
    assert SLIP10Ed25519PublicKey.uncompressed_length() == _["SLIP10-Ed25519"]["uncompressed"]["length"]
    assert SLIP10Ed25519PublicKey.compressed_length() == _["SLIP10-Ed25519"]["compressed"]["length"]
    for public_key_type in ["uncompressed", "compressed"]:
        public_key = SLIP10Ed25519PublicKey.from_bytes(
            get_bytes(_["SLIP10-Ed25519"][public_key_type]["public-key"])
        )
        ...
        assert public_key.raw_uncompressed() == get_bytes(_["SLIP10-Ed25519"]["uncompressed"]["public-key"])
        assert public_key.raw_compressed() == get_bytes(_["SLIP10-Ed25519"]["compressed"]["public-key"])
        assert isinstance(public_key.point(), IPoint)
        assert isinstance(public_key.point(), SLIP10Ed25519Point)


def test_slip10_ed25519_ecc_private_key():

    assert SLIP10Ed25519PrivateKey.name() == _["SLIP10-Ed25519"]["name"]
    assert SLIP10Ed25519PrivateKey.length() == _["SLIP10-Ed25519"]["private-key-length"]
    private_key = SLIP10Ed25519PrivateKey.from_bytes(
        get_bytes(_["SLIP10-Ed25519"]["private-key"])
    )
    ...
    assert private_key.raw() == get_bytes(_["SLIP10-Ed25519"]["private-key"])
    assert isinstance(private_key.public_key(), IPublicKey)
    assert isinstance(private_key.public_key(), SLIP10Ed25519PublicKey)
    assert private_key.public_key().raw_uncompressed() == get_bytes(_["SLIP10-Ed25519"]["uncompressed"]["public-key"])
    assert private_key.public_key().raw_compressed() == get_bytes(_["SLIP10-Ed25519"]["compressed"]["public-key"])
