...
def test_slip10_ed25519_monero_ecc():

    assert SLIP10Ed25519MoneroECC.NAME == _["SLIP10-Ed25519-Monero"]["name"]
...
def test_slip10_ed25519_monero_ecc_point():

    assert SLIP10Ed25519MoneroPoint.name() == _["SLIP10-Ed25519-Monero"]["name"]
    for public_key_type in ["uncompressed", "compressed"]:
        # Test from bytes
        point = SLIP10Ed25519MoneroPoint.from_bytes(
            get_bytes(_["SLIP10-Ed25519-Monero"][public_key_type]["point"]["encode"])
        )
        ...
        assert point.x() == _["SLIP10-Ed25519-Monero"][public_key_type]["point"]["x"]
        assert point.y() == _["SLIP10-Ed25519-Monero"][public_key_type]["point"]["y"]
        assert point.raw_encoded() == get_bytes(_["SLIP10-Ed25519-Monero"][public_key_type]["point"]["encode"])
        assert point.raw() == point.raw_decoded() == get_bytes(_["SLIP10-Ed25519-Monero"][public_key_type]["point"]["decode"])
        # Test from coordinate
        point = SLIP10Ed25519MoneroPoint.from_coordinates(
            x=_["SLIP10-Ed25519-Monero"][public_key_type]["point"]["x"],
            y=_["SLIP10-Ed25519-Monero"][public_key_type]["point"]["y"]
        )
        assert isinstance(point, IPoint)
        assert isinstance(point, SLIP10Ed25519MoneroPoint)
        assert isinstance(point.underlying_object(), bytes)
...
def test_slip10_ed25519_monero_ecc_public_key():

    assert SLIP10Ed25519MoneroPublicKey.name() == _["SLIP10-Ed25519-Monero"]["name"]
    assert SLIP10Ed25519MoneroPublicKey.uncompressed_length() == _["SLIP10-Ed25519-Monero"]["uncompressed"]["length"]
    assert SLIP10Ed25519MoneroPublicKey.compressed_length() == _["SLIP10-Ed25519-Monero"]["compressed"]["length"]
    for public_key_type in ["uncompressed", "compressed"]:
        public_key = SLIP10Ed25519MoneroPublicKey.from_bytes(
            get_bytes(_["SLIP10-Ed25519-Monero"][public_key_type]["public-key"])
        )
        ...
        assert public_key.raw_uncompressed() == get_bytes(_["SLIP10-Ed25519-Monero"]["uncompressed"]["public-key"])
        assert public_key.raw_compressed() == get_bytes(_["SLIP10-Ed25519-Monero"]["compressed"]["public-key"])
        assert isinstance(public_key.point(), IPoint)
        assert isinstance(public_key.point(), SLIP10Ed25519MoneroPoint)


def test_slip10_ed25519_monero_ecc_private_key():

    assert SLIP10Ed25519MoneroPrivateKey.name() == _["SLIP10-Ed25519-Monero"]["name"]
    assert SLIP10Ed25519MoneroPrivateKey.length() == _["SLIP10-Ed25519-Monero"]["private-key-length"]
    private_key = SLIP10Ed25519MoneroPrivateKey.from_bytes(
        get_bytes(_["SLIP10-Ed25519-Monero"]["private-key"])
    )
    ...
    assert private_key.raw() == get_bytes(_["SLIP10-Ed25519-Monero"]["private-key"])
    assert isinstance(private_key.public_key(), IPublicKey)
    assert isinstance(private_key.public_key(), SLIP10Ed25519MoneroPublicKey)
    assert private_key.public_key().raw_uncompressed() == get_bytes(_["SLIP10-Ed25519-Monero"]["uncompressed"]["public-key"])
    assert private_key.public_key().raw_compressed() == get_bytes(_["SLIP10-Ed25519-Monero"]["compressed"]["public-key"])
