...
class TestUnicodeNormalization:
    ...
    def test_character_category_analysis(self):
        """Analyze Unicode character categories before and after normalization.
        
        This test examines the Unicode categories of characters in different scripts
        to understand how NFKD/NFD decomposition affects character composition.
        Categories include:
        - L*: Letters (Lu=uppercase, Ll=lowercase, Lt=titlecase, Lm=modifier, Lo=other)
        - M*: Marks/Combining (Mn=nonspacing, Mc=spacing, Me=enclosing)
        ...
        def analyze_character_categories(text: str, label: str) -> UnicodeAnalysis:
            """Analyze character categories in a text string."""
            categories = []
            category_counts = {}
            scripts = set()
            character_details = []
            ...
            for i, char in enumerate(text):
                ...
                # Try to determine script from character name
        ...
        def compare_normalizations(text: str, label: str):
            """Compare character categories across different normalizations."""
            print(f"\n=== Character Category Analysis: {label} ===")
            print(f"Original text: '{text}'")
            
            # Analyze different normalization forms
            forms = {
                'Original': text,
                'NFC': unicodedata.normalize('NFC', text),
                'NFD': unicodedata.normalize('NFD', text), 
            ...
            for form_name, form_text in forms.items():
                analyses[form_name] = analyze_character_categories(form_text, f"{label}-{form_name}")
            
            # Print summary comparison
            print(f"Length changes: {', '.join(f'{name}={analysis.length}' for name, analysis in analyses.items())}")
            
            # Show category distribution for each form
            for form_name, analysis in analyses.items():
                if analysis.category_counts:
                    categories_str = ', '.join(f"{cat}×{count}" for cat, count in sorted(analysis.category_counts.items()))
                    print(f"{form_name:8}: {categories_str}")
            
            # Show detailed character breakdown for NFD and NFKD (most detailed forms)
            for form_name in ['NFD', 'NFKD']:
                if form_name in analyses:
                    analysis = analyses[form_name]
                    if analysis.character_details:
                        print(f"\n{form_name} detailed breakdown:")
                        for char_info in analysis.character_details:
                            print(f"  '{char_info.char}' {char_info.hex} {char_info.category} - {char_info.name}")
            
            return analyses
        
        print("\n=== Unicode Character Category Analysis ===")
        
        # Test cases covering different script types
        test_cases = [
            # Latin script with accents
            {
                'text': 'café',
                'label': 'Latin with acute accent',
                'expected_decomposition': True
            },
            {
                'text': 'niño', 
                'label': 'Latin with tilde',
                'expected_decomposition': True
            },
            {
                'text': 'résumé',
                'label': 'Latin with multiple accents',
                'expected_decomposition': True
            },
            # Greek script
            {
                'text': 'αβγδε',
                'label': 'Greek basic letters',
                'expected_decomposition': False
            },
            {
                'text': 'άλφα',  # Greek with accent if available
                'label': 'Greek with accent',
                'expected_decomposition': True
            },
            # Cyrillic script
            {
                'text': 'привет',
                'label': 'Cyrillic basic letters',
                'expected_decomposition': False
            },
            # Chinese script
            {
                'text': '中文字符',
                'label': 'Chinese ideographs',
                'expected_decomposition': False
            },
            # Japanese Hiragana
            {
            ...
            {
                'text': 'café²',  # Superscript
                'label': 'Latin with superscript',
                'expected_decomposition': True
            }
        ]
        ...
        for case in test_cases:
            text = case['text']
            label = case['label']
            expected_decomp = case['expected_decomposition']
            
            analyses = compare_normalizations(text, label)
            all_analyses[label] = analyses
            
            # Verify our expectations about decomposition
            original_len = analyses['Original'].length
            nfd_len = analyses['NFD'].length
            nfkd_len = analyses['NFKD'].length
            
            if expected_decomp:
                # Should see length increase or category changes with NFD/NFKD
                has_decomposition = (nfd_len > original_len or 
                                   nfkd_len > original_len or
                                   analyses['NFD'].category_counts != analyses['Original'].category_counts or
                                   analyses['NFKD'].category_counts != analyses['Original'].category_counts)
                assert has_decomposition, f"Expected decomposition for {label} but none found"
            else:
                # Should remain mostly unchanged (unless NFKC/NFKD do compatibility transforms)
                # Note: Some scripts might still have minor changes due to Unicode normalization
                pass  # We'll just observe the results for non-decomposing scripts
        ...
        for label, analyses in all_analyses.items():
            for form_name, analysis in analyses.items():
                all_categories.update(analysis.category_counts.keys())
        
        print(f"All Unicode categories observed: {', '.join(sorted(all_categories))}")
        
        # Count by script type
        script_stats = {}
        for label, analyses in all_analyses.items():
            for script in analyses['Original'].scripts:
                if script not in script_stats:
                    script_stats[script] = []
                script_stats[script].append(label)
        
        print(f"\nScript distribution:")
        for script, labels in script_stats.items():
            print(f"  {script}: {len(labels)} test cases")

    def _determine_script_from_name(self, char_name: str) -> str:
        """Determine script from Unicode character name."""
        name_upper = char_name.upper()
        
        # Script indicators in Unicode names
        script_indicators = {
            'LATIN': 'Latin',
            'GREEK': 'Greek', 
            'CYRILLIC': 'Cyrillic',
        ...
        for indicator, script in script_indicators.items():
            if indicator in name_upper:
                return script
        
        # Check for specific Unicode block ranges
        if not char_name or 'UNKNOWN' in char_name:
            return 'Unknown'
